 ...
 */
static void read_from_mpl(MPU6050_DATA_T *mpu6050_data)
{
    long msg, data[9];
    int8_t accuracy;
...
//				char cStr [ 70 ];
			  ...
			  if (inv_get_sensor_type_euler(data, &accuracy,(inv_time_t*)&timestamp))
				{
            mpu6050_data->MPU6050_Pitch = data[0]*1.0/(1<<16) ;
            mpu6050_data->MPU6050_Roll  = data[1]*1.0/(1<<16);
            mpu6050_data->MPU6050_Yaw   = data[2]*1.0/(1<<16);
            
            mpu6050_data->MPU6050_Gyro_Raw[0]=sensors.gyro.raw[0];
            mpu6050_data->MPU6050_Gyro_Raw[1]=sensors.gyro.raw[1];
            mpu6050_data->MPU6050_Gyro_Raw[2]=sensors.gyro.raw[2];
        
            mpu6050_data->MPU6050_Accel_Raw[0]=sensors.accel.raw[0];
            mpu6050_data->MPU6050_Accel_Raw[1]=sensors.accel.raw[1];
            mpu6050_data->MPU6050_Accel_Raw[2]=sensors.accel.raw[2];
            
            //printf("data[0]->(%ld) [1]->(%ld) [2]->(%ld)\n",data[0],data[1],data[2]);
          
            /*温度*/
            mpu_get_temperature(data,(inv_time_t*)&timestamp);
            mpu6050_data->MPU6050_Temperature=data[0]*1.0/(1<<16);
           // printf("MPU6050_Temperature data[0]->(%ld)",data[0]);
#if 0           
							/*向匿名上位机发送姿态*/
							Data_Send_Status( mpu6050_data->MPU6050_Pitch,mpu6050_data->MPU6050_Roll,mpu6050_data->MPU6050_Yaw);
							/*向匿名上位机发送原始数据*/
...
//							ILI9341_DispString_EN_CH(30,90,(const uint8_t *)cStr, macBACKGROUND,macRED);

//							sprintf ( cStr, "Roll :   %.4f   ", Roll );	//inv_get_sensor_type_euler读出的数据是Q16格式，所以左移16位.
//							ILI9341_DispString_EN_CH(30,110,(const uint8_t *)cStr, macBACKGROUND,macRED);

//							sprintf ( cStr, "Yaw :   %.4f   ", Yaw );	//inv_get_sensor_type_euler读出的数据是Q16格式，所以左移16位.
//							ILI9341_DispString_EN_CH(30,130,(const uint8_t *)cStr, macBACKGROUND,macRED);

							
							
//							sprintf ( cStr, "Temperature :   %.2f   ", data[0]*1.0/(1<<16) );	//inv_get_sensor_type_euler读出的数据是Q16格式，所以左移16位.
//							ILI9341_DispString_EN_CH(30,150,(const uint8_t *)cStr, macBACKGROUND,macRED);
				}
				...
				if (timestamp > hal.next_pedo_ms) {
						...
						dmp_get_pedometer_step_count(&(mpu6050_data->MPU6050_Step_Count));
						dmp_get_pedometer_walk_time(&(mpu6050_data->MPU6050_Walk_Time));
					
//						sprintf(cStr, "Walked steps :  %ld  steps over  %ld  milliseconds..",step_count,walk_time);
//					
//						ILI9341_DispString_EN_CH(0,180,(const uint8_t *)cStr, macBACKGROUND,macRED);
...
#endif  //static void setup_gyro(void)
...
#endif //static inline void run_self_test(void)
...
#endif   //static void handle_input(void)
...
/*******************************************************************************/
static   unsigned long mpu6050_timestamp;
...
{ 
  ...
  if (result)
  {
    ...
    get_tick_count(&mpu6050_timestamp);
...
{    
    ...
    get_tick_count(&mpu6050_timestamp);
         ...
         */
    if (mpu6050_timestamp > hal.next_temp_ms) {
        hal.next_temp_ms = mpu6050_timestamp + TEMP_READ_MS;
        new_temp = 1;
    }
