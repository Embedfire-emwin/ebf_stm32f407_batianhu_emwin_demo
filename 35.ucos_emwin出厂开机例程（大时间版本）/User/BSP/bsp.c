...
void  BSP_Init (void)
{
 ...
 //初始化外部SRAM，已改在启动文件中初始化  
//  FSMC_SRAM_Init();

// SRAM默认不测试	
//	if(SRAM_Test() != 1){bsp_result |= BSP_SRAM;};
...
void Hardware_Error_Check(void)
{
	...
	{
		//只使用核心板，不用底板的时候会有TPAD错误，直接忽略
		if(bsp_result&BSP_TPAD)
		{
			return;
		}
		...
		{
			//打印错误代码
			BSP_TRACE_INFO(("\r\n\r\nerror!bsp_result->(0x%04X)",bsp_result));
			...
			for(i=0;i<(BSP_ERROR_NUM-1);i++)
			{
				...
				{
					BSP_TRACE_INFO(("\r\nhardware init error:%s",&bsp_result_str[i+1][0]));
				}
			}
