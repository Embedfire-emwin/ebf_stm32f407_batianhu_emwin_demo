...
#define COMDATA_MESSAGE_SIZE			1024*5	

static uint8_t *comdata_message;
static GUI_HMEM hMessageMemory;
...
static void LISTVIEW_AddItem(WM_HWIN hWin)
{
  ...
  while((f_result==FR_OK)&&(messageNumRows<MESSAGE_MAX))
  {
    ...
    com_data2null(&comdata_message[0],500);
    f_result = f_read(&f_file,comdata_message, 330, &f_num);
    if((f_result != FR_OK)||(f_num < 30))break;
    comdata_message[30+30]='\0';
    com_gbk2utf8((char *)&comdata_message[30],(char *)&comdata_message[500]);
    _aTable_[0][0]=(char *)&comdata_message[0];
		_aTable_[0][1]=(char *)&comdata_message[10];
		_aTable_[0][2]=(char *)&comdata_message[500];
...
{    
  sprintf((char *)&comdata_message[0],UTF8_UNREAD);
  sprintf((char *)&comdata_message[10],"%s",num);
  sprintf((char *)&comdata_message[30],"%s",gbkstr);
  comdata_message[329]='\n';
  com_gbk2utf8((char *)&comdata_message[30],(char *)&comdata_message[500]);
  comdata_message[500+30]='\0';
  _aTable_[0][0]=(char *)&comdata_message[0];
  _aTable_[0][1]=(char *)&comdata_message[10];
  _aTable_[0][2]=(char *)&comdata_message[500];
  LISTVIEW_AddRow(WM_GetDialogItem(hWin, GUI_ID_LISTVIEW0), _aTable_[0]);
  sprintf((char *)&comdata_message[0],UTF8_FROM);
  ...
  f_result = f_lseek(&f_file,messageNumRows*330); 
  f_result = f_write(&f_file, (char *)&comdata_message[0], 330, &f_num);
  messageNumRows++;
  f_close(&f_file); 
}	
// USER END
  ...
  case WM_NOTIFY_PARENT:
    ...
    case GUI_ID_LISTVIEW0: // Notifications sent by 'MessageList'
      ...
      case WM_NOTIFICATION_RELEASED:
        ...
        if(f_result==FR_OK)
        {
          ...
          EDIT_SetText(WM_GetDialogItem(pMsg->hWin, GUI_ID_EDIT0), (char *)&comdata_message[10]);
          com_gbk2utf8((char *)&comdata_message[30],(char *)&comdata_message[500]);
          MULTIEDIT_SetText(WM_GetDialogItem(pMsg->hWin, GUI_ID_MULTIEDIT0), (char *)&comdata_message[500]);
        }
        f_close (&f_file);
        // USER END
        break;
    ...
    case GUI_ID_BUTTON0: // Notifications sent by 'Send'
      ...
      case WM_NOTIFICATION_RELEASED:
        // USER START (Optionally insert code for reacting on notification message)
        //OSSchedLock(&err);
        com_data2null(&comdata_message[0],1000);
        EDIT_GetText(WM_GetDialogItem(pMsg->hWin, GUI_ID_EDIT0), (char *)&comdata_message[10],20);
        hItem=WM_GetDialogItem(pMsg->hWin, GUI_ID_MULTIEDIT0);
        MULTIEDIT_GetText(hItem,(char *)&comdata_message[500],MULTIEDIT_GetTextSize(hItem));
        if(gsm_flag==1)
        {
					uint8_t result;
          result = gsm_sms_utf8((char *)&comdata_message[10],
																	(char *)&comdata_message[500],
																	strlen((char *)&comdata_message[10]),
        ...
        sprintf((char *)&comdata_message[0],UTF8_TO); 
        com_utf82gbk((char *)&comdata_message[500],(char *)&comdata_message[30]);
        comdata_message[329]='\n';
        comdata_message[500+30]='\0';
        _aTable_[0][0]=(char *)&comdata_message[0];
        _aTable_[0][1]=(char *)&comdata_message[10];
...
//  OS_ERR  err;
  ...
  char messagename[20];
	...
	//申请空间
	hMessageMemory = GUI_ALLOC_AllocZero(COMDATA_MESSAGE_SIZE);
  comdata_message = GUI_ALLOC_h2p(hMessageMemory);
...
//        OSSchedLock(&err);
        GSM_CLEAN_RX();//清除接收缓存
        com_data2null(&comdata_message[1000],2000);
        GSM_DELAY(100);
				IsRead=readmessage(newmessadd,(char *)&comdata_message[1000],(char *)&comdata_message[1200]); 
				printf("IsRead->(%d)\n",IsRead);
        if(IsRead)
				{
					hexuni2gbk((char *)&comdata_message[1000],messagename);	
					hexuni2gbk((char *)&comdata_message[1200],(char *)&comdata_message[2000]);	
          printf("name->(%s) mes->(%s)\n",messagename,(char *)&comdata_message[2000]);          
					saveReceiveMessage(hWin,messagename,(char *)&comdata_message[2000]);					
				}     
        //OSSchedUnlock(&err);
