...
/**
  ...
  * @retval 无
  */
void ColorLED_TurnOFF(void)
{
	SetColorValue(0,0,0);
	//关闭通道
	COLOR_RED_TIM_FORCED_CONFIG_FUN(COLOR_RED_TIM,TIM_ForcedAction_InActive);
	COLOR_GREEN_TIM_FORCED_CONFIG_FUN(COLOR_GREEN_TIM,TIM_ForcedAction_InActive);
	COLOR_BLUE_TIM_FORCED_CONFIG_FUN(COLOR_BLUE_TIM,TIM_ForcedAction_InActive);
	...
	// 关闭计数器
	TIM_Cmd(COLOR_RED_TIM, DISABLE);		
	TIM_Cmd(COLOR_GREEN_TIM, DISABLE);
	TIM_Cmd(COLOR_BLUE_TIM, DISABLE);
  
	//关闭时钟						
	COLOR_RED_TIM_APBxClock_FUN(COLOR_RED_TIM_CLK,DISABLE);
	COLOR_GREEN_TIM_APBxClock_FUN(COLOR_GREEN_TIM_CLK,DISABLE);
	COLOR_BLUE_TIM_APBxClock_FUN(COLOR_BLUE_TIM_CLK,DISABLE);
