...
void GUIDEMO_DispTitle(char * pTitle) {
  ...
  xSize     = LCD_GetXSize();
  if ((xSize - StringLen) / 2 - 5 < bmSTLogo70x35.XSize + LOGO_DIST_BORDER)  {
    RectTitle.x0 = bmSTLogo70x35.XSize + LOGO_DIST_BORDER;
  } else {
    RectTitle.x0 = 0;
  }
  RectTitle.y0 = 0;
  RectTitle.x1 = xSize - 1;
  RectTitle.y1 = bmSTLogo70x35.YSize + 2 * LOGO_DIST_BORDER - 1;
  GUI_SetTextMode(GUI_TM_TRANS);
...
int GUIDEMO_GetTitleSizeY(void) {
  return bmSTLogo70x35.YSize + 2 * LOGO_DIST_BORDER;
}
