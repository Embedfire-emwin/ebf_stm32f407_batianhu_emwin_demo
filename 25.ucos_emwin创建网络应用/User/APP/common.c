...
static const GUI_WIDGET_CREATE_INFO _aDialogCreateError[] = {
  { FRAMEWIN_CreateIndirect, "Error",  ERROR_FRAMEWIN_0,  0,   0,   310, 160, 0, 0x0, 0 },
  { BUTTON_CreateIndirect,   "CLOSE",  ERROR_BUTTON_0,    60,  80,  60,  30,  0, 0x0, 0 },
...
static void _cbDialogError(WM_MESSAGE * pMsg) {
  ...
  case WM_PAINT:
      ...
      GUI_DispStringHCenterAt(ErrorStr[0],150,5);      
      GUI_DispStringHCenterAt(ErrorStr[1],150,35);
   break;
  // USER END
  default:
    WM_DefaultProc(pMsg);
...
void ErrorDialog(WM_HWIN hParent,char *string1,char *string2)
{
  ...
  ErrorStr[0]=string1;
  ErrorStr[1]=string2;
  hWin=GUI_CreateDialogBox(_aDialogCreateError,GUI_COUNTOF(_aDialogCreateError),_cbDialogError,hParent,85,300);
  WM_MakeModal(hWin);
