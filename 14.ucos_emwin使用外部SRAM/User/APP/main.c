...
static  void  AppTaskCreate (void)
{
	...
	OSTaskCreate((OS_TCB     *)&AppTaskGUIDemoTCB,             // 任务TCB                               
							 (CPU_CHAR   *)"GUI Demo Test", 									// 任务名称                             
							 (OS_TASK_PTR ) GUIDEMO_Main,									  // 任务函数指针                                
							 (void       *) 0,																	// 可选输入数据
							 (OS_PRIO     ) APP_TASK_GUI_DEMO_PRIO,					// 优先级
							 (CPU_STK    *)&AppTaskGUIDemoStk[0],							// 任务栈基地址
							 (CPU_STK_SIZE) APP_TASK_GUI_DEMO_STK_SIZE / 10,				// 栈“水印”限制
							 (CPU_STK_SIZE) APP_TASK_GUI_DEMO_STK_SIZE,        		//栈大小
							 (OS_MSG_QTY  ) 0u,
							 (OS_TICK     ) 0u,
							 (void       *) 0,
							 (OS_OPT      )(OS_OPT_TASK_STK_CHK | OS_OPT_TASK_STK_CLR),//可选配置
