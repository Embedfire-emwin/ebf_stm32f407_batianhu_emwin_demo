    ...
    if(NCode==WM_NOTIFICATION_RELEASED)
    {
      switch(Id)
      {
        case ERROR_BUTTON_0: // Notifications sent by 'CLOSE'
					...
					//先判断父窗口是否为桌面
					if(WM_GetParent(pMsg->hWin) == WM_HBKWIN)
					{
...
//static FATFS   fs[2] __EXRAM;													  /* Work area (file system object) for logical drives */

//static FRESULT f_result; 
//static FIL     f_file __EXRAM;
//static UINT    f_num;
...
void emWin_CreateBMPPicture(uint8_t * FileName, int x0, int y0, int xSize, int ySize)
{
   static uint8_t init_flag = 0;
	...
	if(init_flag == 0)
	{
		f_result = f_mount(&fs[1],"0:",1);
		if(f_result != FR_OK) goto createBMP_failure;
