...
uint8_t tcp_echoclient_connect(DRV_NETWORK network)
{
  ...
  if (echoclient_pcb != NULL)
  {
    ...
    err=tcp_connect(echoclient_pcb,&DestIPaddr,network.net_remote_port,tcp_echoclient_connected);
    if(err==ERR_OK)
    {
      return 0;
    }
...
void network_tcpclient_send(char *str)
{  
  if (echoclient_es->state == ES_CONNECTED)
  {    
    sprintf((char*)data, "%s", str);
      
    /* allocate pbuf */
    echoclient_es->p_tx = pbuf_alloc(PBUF_TRANSPORT, strlen((char*)data) , PBUF_POOL);
       
    if (echoclient_es->p_tx)
    {       
      /* copy data to pbuf */
      pbuf_take(echoclient_es->p_tx, (char*)data, strlen((char*)data));  
      /* send data */
      tcp_echoclient_send(echoclient_pcb,echoclient_es);
    }
  }
}
