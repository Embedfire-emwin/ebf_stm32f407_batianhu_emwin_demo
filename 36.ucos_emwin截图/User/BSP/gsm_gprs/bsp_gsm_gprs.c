...
extern void com_data2null(uint8_t *data,uint32_t length);


void gsm_utf82ucs2hex(char * ucs2hex,char * utf8,uint16_t utf8len)
{
    char *ucs,*tmp;//ucs用于存储unicode变量，tmp用作unicode变量的指针运算，如果运算后free tmp会出错
		uint16_t i=0;
	
		ucs	= (char *)malloc(sizeof(char)*utf8len*2+2);		//UC编码全为2字节一个,再加上2个结束符			
		...
		{
			*(WCHAR *)tmp = GSM_SWAP16(*(WCHAR *)tmp);
			
			if(((*tmp)&0xFF) < 0x7F)      //英文
        {					
					gsm_char2hex(ucs2hex,*tmp);
					ucs2hex+=2;
					tmp++;
...
//void gsm_sms_utf8(char *num,char *smstext,uint16_t numlen,uint16_t textlen)
uint8_t gsm_sms_utf8(char *num,char *smstext,uint16_t numlen,uint16_t textlen)
{
	  char converbuff[160];
	  char cmdbuff[160];
	  char end= 0x1A;
    ...
    if(IsASSIC(smstext)==GSM_TRUE)
    {
			//英文
			//"GSM"字符集
			if(gsm_cmd("AT+CSCS=\"GSM\"\r","OK", 100) != GSM_TRUE) goto sms_utf8_failure;

			//文本模式
			if(gsm_cmd("AT+CMGF=1\r","OK", 100) != GSM_TRUE) goto sms_utf8_failure;
    ...
    else
    {
			//中文
			//"UCS2"字符集
			if(gsm_cmd("AT+CSCS=\"UCS2\"\r","OK", 100) != GSM_TRUE) goto sms_utf8_failure;
			//文本模式
			if(gsm_cmd("AT+CMGF=1\r","OK", 100) != GSM_TRUE) goto sms_utf8_failure;

			if(gsm_cmd("AT+CSMP=17,167,0,8\r","OK", 100) != GSM_TRUE) goto sms_utf8_failure;
	
			
			GSM_DEBUG("num :%s,numlen :%d",num,numlen);

			//生成电话号码命令utf8-ucs，UCS2的电话号码(需要转成 ucs2码)
			gsm_utf82ucs2hex(converbuff,num,numlen);
			
		GSM_DEBUG("UCS2hex");
		GSM_DEBUG_ARRAY((uint8_t*)converbuff,numlen*6);

			
			GSM_DEBUG("num converbuff:%s",converbuff);
			sprintf(cmdbuff,"AT+CMGS=\"%s\"\r",converbuff);  
			//转换短信内容
      gsm_utf82ucs2hex(converbuff,smstext,textlen);
			GSM_DEBUG("text converbuff:%s",converbuff);
