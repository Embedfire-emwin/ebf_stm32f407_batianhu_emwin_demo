...
static  void  AppTaskCreate (void)
{
	OS_ERR  os_err;
		...
		//创建应用任务,测试Fatfs
	OSTaskCreate((OS_TCB     *)&AppTaskFatfsTestTCB,             // 任务TCB                               
							 (CPU_CHAR   *)"GUI FatFs Test", 									// 任务名称                             
							 (OS_TASK_PTR ) Fatfs_MainTask,									  // 任务函数指针                                
							 (void       *) 0,																	// 可选输入数据
							 (OS_PRIO     ) APP_TASK_FATFS_TEST_PRIO,					// 优先级
							 (CPU_STK    *)&AppTaskFatfsTestStk[0],							// 任务栈基地址
							 (CPU_STK_SIZE) APP_TASK_FATFS_TEST_STK_SIZE / 10,				// 栈“水印”限制
							 (CPU_STK_SIZE) APP_TASK_FATFS_TEST_STK_SIZE,        		//栈大小
							 (OS_MSG_QTY  ) 0u,
							 (OS_TICK     ) 0u,
							 (void       *) 0,
							 (OS_OPT      )(OS_OPT_TASK_STK_CHK | OS_OPT_TASK_STK_CLR),//可选配置
