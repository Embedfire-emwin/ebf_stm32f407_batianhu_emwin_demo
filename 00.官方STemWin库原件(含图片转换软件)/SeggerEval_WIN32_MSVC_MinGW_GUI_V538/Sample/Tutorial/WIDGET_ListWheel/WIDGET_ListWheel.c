...
static void _cbBkWheel(WM_MESSAGE * pMsg) {
  ...
  case WM_PAINT:
    ...
    GUI_DrawGradientV(0, 0, xSize - 1, ySize - 1, 0xb1b1b1, 0x0d0d0d);
    break;
  default:
    WM_DefaultProc(pMsg);
  }
...
static void _CreateAreaDevices(int xPos, int yPos, int xSize, int ySize, int r, int Location, GUI_MEMDEV_Handle * phMem) {
  ...
  *(phMem + 0) = GUI_MEMDEV_CreateFixed(xPos, yPos, xSize, ySize, GUI_MEMDEV_NOTRANS, GUI_MEMDEV_APILIST_32, GUI_COLOR_CONV_8888);
  *(phMem + 1) = GUI_MEMDEV_CreateFixed(xPos, yPos, xSize, ySize, GUI_MEMDEV_NOTRANS, GUI_MEMDEV_APILIST_32, GUI_COLOR_CONV_8888);
  for (i = 0; i < 2; i++) {
    hMemPrev = GUI_MEMDEV_Select(*(phMem + i));
    if (i) {
      GUI_DrawGradientV(0, 0, xSize - 1, ySize - 1, 0xFFBB55, LIGHTBLUE);
    } else {
...
static void _cbBkWin(WM_MESSAGE * pMsg) {
  ...
  case WM_PAINT:
    ...
    // Fill background
    //
    GUI_DrawGradientV(0,   0, 239,  23, 0xCCCCCC, 0xCCCCCC);
    GUI_DrawGradientV(0,  24, 239,  24, 0x666666, 0x666666);
    GUI_DrawGradientV(0,  25, 239,  25, 0xDDDDDD, 0xDDDDDD);
    GUI_DrawGradientV(0,  26, 239,  68, 0xCCCCCC, 0xBBAA88);
    GUI_DrawGradientV(0,  69, 239,  69, 0x666666, 0x666666);
    GUI_DrawGradientV(0,  70, 239, 158, 0xCCCCCC, 0xCCCCCC);
    GUI_DrawGradientV(0, 159, 239, 159, 0x666666, 0x666666);
    GUI_DrawGradientV(0, 160, 239, 160, 0xDDDDDD, 0xDDDDDD);
    GUI_DrawGradientV(0, 161, 239, 319, 0xBBBBBB, 0x000000);
    //
    // Draw logo
    //
  ...
  *(phMem + 1) = GUI_MEMDEV_CreateFixed(xPos, yPos, xSize, ySize, GUI_MEMDEV_NOTRANS, GUI_MEMDEV_APILIST_32, GUI_COLOR_CONV_8888);
  for (i = 0; i < 2; i++) {
    ...
    if (i) {
      ...
      GUI_DrawGradientV(0, 0, xSize - 1, ySize - 1, LIGHTBLUE, 0xFFBB55);
    }
    GUI_SetColor(GUI_LIGHTGRAY);
    GUI_DrawRoundedRect(0, 0, xSize - 1, ySize - 1, r);
    GUI_SetColor(GUI_GRAY);
