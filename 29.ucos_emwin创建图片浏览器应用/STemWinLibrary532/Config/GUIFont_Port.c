...
static int _cb_FONT_XBF_GetData(U32 Offset, U16 NumBytes, void * pVoid, void * pBuffer)
{
	...
	if (storage_init_flag == 0)
	{
		...
		//读取字库文件的前几个字节，以检查字库文件是否正常
		SPI_FLASH_BufferRead(data,FONT_XINSONGTI_25_ADDR,2);
		SPI_FLASH_BufferRead(&data[2],FONT_XINSONGTI_19_ADDR,2);
...
{	
		uint8_t create_res = 0;
	
		//
		// 新宋体25
		//
		create_res = GUI_XBF_CreateFont(&FONT_XINSONGTI_25,    					// GUI_FONT 字体结构体指针
		...
		// 新宋体19
		//
		create_res = GUI_XBF_CreateFont(&FONT_XINSONGTI_19,    				// GUI_FONT 字体结构体指针
